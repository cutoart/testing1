...
export default {
  ...
  created() {
    if (this.id != undefined && this.id != null) {
      ...
      document.title = this.$store.state.projecttitle + " - Edit Promotion";
      APIService.getById("/PromotionGetById", this.id).then((response) => {
        Store.commit("hideLoader");
        this.$set(this, "vmodel", response.data);
        this.image =
          this.$store.state.apiURL + "/promotion/" + this.vmodel.image;
      });
    } else {
      document.title = this.$store.state.projecttitle + " - Add Promotion";
    }
  },
  ...
  methods: {
    ...
    handleSubmit() {
      ...
      if (this.id != undefined && this.id != null && this.id != "") {
        APIService.post("/UpdatePromotion", _vmodel, this.id).then(() => {
          this.$successAlert("Your data is updated.");
          this.$router.push({ name: "PromotionList" });
        });
      } else {
        APIService.post("/AddPromotion", this.vmodel).then(() => {
