    ...
    <CCol>
      <CCard>
        <CForm @submit.prevent="handleSubmit">
          <CCardHeader>
            <strong>BoxStorage Order Received Html</strong>
          </CCardHeader>
          <CCardBody>
            <CRow>
              <CCol sm="6">
...
export default {
  ...
  created() {
    ...
    document.title =
      this.$store.state.projecttitle + " - BoxStorage Order Received Html";
    APIService.post("/GetEmailTemplateByType", {
      type: "BoxStorageOrderReceived",
    }).then((response) => {
  ...
  methods: {
    handleSubmit() {
      ...
      this.vmodel.type = "BoxStorageOrderReceived";
      APIService.post("/AddUpdateEmailTemplate", _vmodel).then(() => {
        this.$router.go({ name: "BoxStorage Order Received" });
