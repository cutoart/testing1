    ...
    <CCol>
      <CCard>
        <CForm @submit.prevent="handleSubmit">
          <CCardHeader>
            <strong>SelfStorage Refund Html</strong>
          </CCardHeader>
          <CCardBody>
            <CRow>
              <CCol sm="6">
...
export default {
  ...
  created() {
    ...
    document.title =
      this.$store.state.projecttitle + " - SelfStorage Refund Html";
    APIService.post("/GetEmailTemplateByType", {
      type: "SelfStorageRefund",
    }).then((response) => {
  ...
  methods: {
    handleSubmit() {
      ...
      this.vmodel.type = "SelfStorageRefund";
      APIService.post("/AddUpdateEmailTemplate", _vmodel).then(() => {
        this.$router.go({ name: "SelfStorage Refund" });
