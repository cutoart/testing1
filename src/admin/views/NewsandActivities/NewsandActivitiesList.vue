  ...
  <div>
    <CCard>
      <CCardHeader>
        <CRow>
          <CCol>
            <div class="hello">List NewsandActivities</div>
          </CCol>
          <CCol col="3">
            <CButton
              block
              router-link
              :to="{ name: 'AddNewsandActivities' }"
              color="dark"
              >Add News & Activities</CButton
            >
          </CCol>
        </CRow>
      </CCardHeader>
        ...
        >
          <template #image="{ item }">
            <td style="text-align:center;">
              <img
                :src="$store.state.apiURL + '/newsandactivities/' + item.image"
                height="100"
                width="100"
              />
            </td>
...
export default {
  ...
  created() {
    ...
    this.getNewsandActivitiesListData();
  },

  methods: {
    getNewsandActivitiesListData() {
      APIService.post(
    ...
    deActivateItem(id, deleted) {
      ...
      this.$confirm({
        ...
        callback: (confirm) => {
          if (confirm) {
            APIService.delete("/DeActivateNewsAndActivities", req).then(
              (response) => {
                this.$set(this, "item", response.data);
              }
            );
    ...
    deleteItem(id) {
      ...
      this.$confirm({
        ...
        callback: (confirm) => {
          if (confirm) {
            APIService.delete("/DeleteNewsAndActivities", req).then(
              (response) => {
                this.$set(this, "item", response.data);
              }
            );
