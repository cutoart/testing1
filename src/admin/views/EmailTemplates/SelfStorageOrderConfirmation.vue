    ...
    <CCol>
      <CCard>
        <CForm @submit.prevent="handleSubmit">
          <CCardHeader>
            <strong>SelfStorage Order Confirmation Html</strong>
          </CCardHeader>
          <CCardBody>
            <CRow>
              <CCol sm="6">
...
export default {
  ...
  created() {
    ...
    document.title =
      this.$store.state.projecttitle + " - SelfStorage Order Confirmation Html";
    APIService.post("/GetEmailTemplateByType", {
      type: "SelfStorageOrderConfirmation",
    }).then((response) => {
  ...
  methods: {
    handleSubmit() {
      ...
      this.vmodel.type = "SelfStorageOrderConfirmation";
      APIService.post("/AddUpdateEmailTemplate", _vmodel).then(() => {
        this.$router.go({ name: "SelfStorage Order Confirmation" });
