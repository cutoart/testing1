  ...
  <div>
    <CCard>
      <CCardHeader>
        <CRow>
          <CCol>
            <div class="hello">List Promotion</div>
          </CCol>
          <CCol col="2">
            <CButton
              block
              router-link
              :to="{ name: 'AddPromotion' }"
              color="dark"
              >Add Promotion</CButton
            >
          </CCol>
        </CRow>
      </CCardHeader>
        ...
        >
          <template #image="{ item }">
            <td style="text-align: center">
              <img
                :src="$store.state.apiURL + '/promotion/' + item.image"
                height="100"
                width="100"
              />
            </td>
...
export default {
  ...
  methods: {
    getPromotionListData() {
      APIService.post("/ListOfPromotion", { usertype: "admin" }, null).then(
        ((response) => {
    ...
    deActivateItem(id, deleted) {
      ...
      this.$confirm({
        ...
        callback: (confirm) => {
          if (confirm) {
            APIService.delete("/DeActivatePromotion", req).then((response) => {
              this.$set(this, "item", response.data);
            });
          }
        },
    ...
    deleteItem(id) {
      ...
      this.$confirm({
        ...
        callback: (confirm) => {
          if (confirm) {
            APIService.delete("/DeletePromotion", req).then((response) => {
              this.$set(this, "item", response.data);
            });
          }
        },
