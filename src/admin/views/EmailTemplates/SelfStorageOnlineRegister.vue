    ...
    <CCol>
      <CCard>
        <CForm @submit.prevent="handleSubmit">
          <CCardHeader>
            <strong>SelfStorage Online Registration</strong>
          </CCardHeader>
          <CCardBody>
            <CRow>
              <CCol sm="6">
            ...
            <CRow>
              <CCol sm="6">
                <CInput
                  ...
                  v-on:keyup="vmodel.email = $event.target.value.toLowerCase()"
              ...
              <CCol sm="12" class="mt-2">
                <label
                  >HTML Template for User <br />
                  <small>Note: {{ note }} </small></label
                >
                <ckeditor v-model="vmodel.userhtml"></ckeditor>
              </CCol>
            </CRow>
          </CCardBody>
          <CCardFooter>
...
export default {
  ...
  created() {
    ...
    document.title =
      this.$store.state.projecttitle + " - SelfStorage Online Register";
    APIService.post("/GetEmailTemplateByType", {
      type: "SelfStorageOnlineRegister",
    }).then((response) => {
  ...
  methods: {
    ...
    handleSubmit() {
      ...
      this.vmodel.type = "SelfStorageOnlineRegister";
      APIService.post("/AddUpdateEmailTemplate", _vmodel).then(() => {
        this.$router.go({ name: "SelfStorage Online Register" });
