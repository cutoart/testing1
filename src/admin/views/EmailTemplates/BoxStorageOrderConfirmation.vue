    ...
    <CCol>
      <CCard>
        <CForm @submit.prevent="handleSubmit">
          <CCardHeader>
            <strong>BoxStorage Order Confirmation Html</strong>
          </CCardHeader>
          <CCardBody>
            <CRow>
              <CCol sm="6">
...
export default {
  ...
  created() {
    ...
    document.title =
      this.$store.state.projecttitle + " - BoxStorage Order Confirmation Html";
    APIService.post("/GetEmailTemplateByType", {
      type: "BoxStorageOrderConfirmation",
    }).then((response) => {
  ...
  methods: {
    handleSubmit() {
      ...
      this.vmodel.type = "BoxStorageOrderConfirmation";
      APIService.post("/AddUpdateEmailTemplate", _vmodel).then(() => {
        this.$router.go({ name: "BoxStorage Order Confirmation" });
